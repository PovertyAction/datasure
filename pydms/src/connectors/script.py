...
def script_add_form() -> None:
	...
	# input file alias
	script_alias = st.text_input(label = "alias*", 
								help = "Enter a unique name for the file", 
								key = "script_alias_key")

	# file uploader. Limit to 1 file and allow only file types selected
	added_script = st.text_input(label = "file path*", 
							  			help = "Add full file name and path. eg. C:/data/kobo_import.py", 
										key = "added_script_key")
	...
	# get name of dataset to input from script
	added_script_data = st.text_input(label = "Dataset*", 
							  			help = "Specify the dataset to be loaded from script", 
										key = "added_script_data_key")
	...
	# add a submit button
	add_script_btn = st.button("Add Script", type = "primary", 
											 use_container_width = True,
											 key = "add_script_btn_key",
											 disabled = not added_script or not script_alias or not added_script_data)
	# add_file action
	if add_script_btn:
		...
		new_script = pd.DataFrame(data = [[script_alias, True, added_script_ext, added_script, added_script_data]], 
										columns = ['alias', 'load', 'type', 'filename', 'dataset'])
				
		st.session_state.script_files = pd.concat([st.session_state.script_files, new_script], ignore_index = True)
...
def script_load_action(script_inputs: dict) -> None:
	...
	# remove empty rows
	script_inputs = script_inputs[script_inputs['load'] == True]
	...
	for i in range(0, form_count):
		...
		if f'script_raw_data{i}' in st.session_state:
			...
			st.write('Name', exec(open(filename).read()))
