...
def local_add_form() -> None:
	...
	# input file alias
	local_file_alias = st.text_input(label = "alias*", 
								help = "Enter a unique name for the file")

	# file uploader. Limit to 1 file and allow only file types selected
	local_added_file = st.text_input(label = "file path*", 
	...
	if local_added_file:
		...
		if not os.path.isfile(local_added_file):
			...
			local_added_file_sheet_name = st.selectbox(label = "Sheet Name", 
										options = sheets)
		else:
			local_added_file_sheet_name = None

	# add a submit button
	local_add_file = st.button("Add File", type = "primary", 
	...
	if local_add_file:
		...
		new_file = pd.DataFrame(data = [[local_file_alias, True, local_added_file_ext, local_added_file, local_added_file_sheet_name]], 
								columns = ['alias', 'load', 'type', 'filename', 'sheet name'])
		
		#cst.write(new_file)
		
		st.session_state.local_files = pd.concat([st.session_state.local_files, new_file], ignore_index = True)	
...
def local_load_action(local_inputs: pd.DataFrame) -> None:
	...
	# remove empty rows
	local_inputs = local_inputs[local_inputs['load'] == True]
	...
	if local_inputs.empty:
		...
		for i in range(0, local_files_count):
			if f'local_raw_data{i}' in st.session_state:
				...
				st.session_state[f'local_raw_data{i}'] = local_read_data(local_filename, local_sheet_name)
				time.sleep(1)
