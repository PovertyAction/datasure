...
=======
    ...
    with script_add_col, st.container(border=True):
        script_add_form()

    with script_show_col, st.container(border=True):
        if len(st.session_state.script_files.index) > 0:
            st.session_state.script_disable_load = False

            script_inputs_mod = st.data_editor(
                data=st.session_state.script_files,
                use_container_width=True,
                num_rows="dynamic",
            )

            # Save configuration File
            script_save_config = st.button(
                "Save setting", type="secondary", key="script_save_config_key"
            )
    ...
    if st.session_state.script_show_preview:
        with st.container(border=True):
            ...
            st.session_state.script_alias_list = st.session_state.script_files[
                "alias"
            ].tolist()
            st.session_state.alias_list_index[3] = len(
                st.session_state.script_alias_list
            )

            with script_select_col:
                script_preview_data = st.selectbox(
                    "Select Dataset to preview:",
                    options=st.session_state.script_alias_list,
                )
                script_row_num = st.session_state["script_alias_list"].index(
                    script_preview_data
                )

            script_row_count: int = len(
                st.session_state[f"script_raw_data{script_row_num}"].index
            )
            script_col_count: int = len(
                st.session_state[f"script_raw_data{script_row_num}"].columns
            )
            script_miss_count: int = (
                st.session_state[f"script_raw_data{script_row_num}"]
                .isnull()
                .sum()
                .sum()
            )
            script_miss_perc: float = round(
                (script_miss_count / (script_row_count * script_col_count)) * 100, 2
            )
