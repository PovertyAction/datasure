...
def donut_chart(actual_value: int, target_value: int = 100, title: str = None, prefix: str = "", 
                suffix: str = "%", colours: list = ['#2C5F2D', '#CCCCCC']):
    """
    ...
    fig: matplotlib figure
        The created figure
    """
    fig = plt.figure(figsize=(2, 2), dpi=100, facecolor='#FFFFFF', constrained_layout=True)
    ...
    pie = ax.pie([actual_value, target_value - actual_value], 
                colors=colours, 
                startangle=90, 
                labeldistance=1.15, 
                counterclock=False)
...
def progress_report(data, page_num) -> None:
    ...
    with met_col1:
        st.metric(
            ...
            value=total_goal if 'total_goal_progress' in st.session_state and st.session_state['total_goal_progress'] > 0 else "N/A"
        )
    
    with met_col2:
        st.metric(
            label="Total Submitted Interviews",
            value=total_submitted
        )
    ...
    # Consent chart
    with chart_cols[0]:
        if consent and 'consent_val_progress' in st.session_state and len(st.session_state['consent_val_progress']) > 0:
            # Count total valid consents
            valid_consent_count = data[data[consent].isin(st.session_state['consent_val_progress'])][survey_id].nunique()
            consent_percentage = round((valid_consent_count / total_submitted * 100), 0) if total_submitted > 0 else 0
            
            # Create matplotlib donut chart for consent
            st.markdown("<p font-size: 16px;'>Valid Consent</p>", unsafe_allow_html=True)
            fig = donut_chart(
    ...
    # Outcome chart
    with chart_cols[1]:
        if outcome and 'outcome_val_progress' in st.session_state and len(st.session_state['outcome_val_progress']) > 0:
            # Count total completed surveys
            completed_count = data[data[outcome].isin(st.session_state['outcome_val_progress'])][survey_id].nunique()
            completion_percentage = round((completed_count / total_submitted * 100), 0) if total_submitted > 0 else 0
            
            # Create matplotlib donut chart for outcome
            st.markdown("<p font-size: 16px;'>Survey Completion</p>", unsafe_allow_html=True)
            fig = donut_chart(
    ...
    with col1:
        ...
        summary = data.groupby(consent,observed=True)[survey_id].nunique().reset_index()
    ...
    with col2:
        # Group by 'id' and count unique values of "key"
            ...
            fig = go.Figure(data=[go.Pie(labels=count_unique_ids.unique_key_count, values=count_unique_ids[survey_id], hole=.3, marker=dict(colors=colors))])
            ...
            count_by_date = data.groupby('date_only').size().reset_index(name='num_interviews')
    ...
    with col3:
        ...
        table_data['number_of_attempts'] = table_data[survey_id].map(unique_counts.set_index(survey_id)['unique_key_count'])
    ...
    period_stats = chart_data.groupby('time_period').agg(
