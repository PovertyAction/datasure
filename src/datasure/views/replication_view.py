...
Allows researchers to export a fully structured, reproducible replication
package (raw data, correction scripts, corrected output, codebook, README)
as a zip file for download to their local drive.
"""
...
st.markdown(
    "Export a self-contained replication package that allows anyone to "
...
with col_right:
    survey_name = st.text_input(
        ...
        help="Used to name the raw and corrected CSV files.",
    )
...
lang_display = st.radio(
    ...
    options=_LANG_OPTS,
    horizontal=True,
    help="The scripting language used to generate the correction scripts.",
)
...
with st.expander("What's inside the zip?", expanded=False):
    if selected_alias and project_name and survey_name:
        ...
        ext = LANGUAGE_EXT.get(lang_display.lower(), "py")
        st.code(
