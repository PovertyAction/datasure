...
class PrepDescriptions:
    ...
    def get_description(cls, category: str, function: str) -> str | None:
        ...
        return category_map.get(category, {}).get(function.lower())
...
class PrepConfirmationMessages:
    ...
    @staticmethod
    def _format_column_names(column_names: str | list[str]) -> str:
    ...
    # Main Actions
    @classmethod
    def transform_column(cls, result: PrepActionResult) -> str:
        ...
        return (f"✓ Column transformation applied. {column_display} updated using "
                f"{result.method}. {result.affected_count} {row_text} affected.")

    @classmethod
    def add_new_column(cls, result: PrepActionResult) -> str:
        """Generate message for adding new column."""
        source_display = cls._format_column_names(result.source_columns) if result.source_columns else "specified parameters"
        column_text = cls._pluralize(result.remaining_count, "column")
        return (f'✓ New column {cls._format_column_names(result.column_names)} added. '
    ...
    def remove_columns(cls, result: PrepActionResult) -> str:
        ...
        column_count = len(result.column_names) if isinstance(result.column_names, list) else 1
        column_text = cls._pluralize(column_count, "column")
        remaining_text = cls._pluralize(result.remaining_count, "column")
        column_display = cls._format_column_names(result.column_names)
        return (f"✓ {column_count} {column_text} removed. {column_display} deleted from "
    ...
    def remove_rows(cls, result: PrepActionResult) -> str:
        ...
        return (f"✓ {result.affected_count} {row_text} removed. Deleted rows {method_text}. "
    ...
    def add_column_constant(cls, result: PrepActionResult) -> str:
        ...
        row_text = cls._pluralize(result.affected_count, "row")
        return (f'✓ Constant column added. {cls._format_column_names(result.column_names)} '
    ...
    def add_column_calculation(cls, result: PrepActionResult) -> str:
        ...
        return (f'✓ {method_name} column added. {cls._format_column_names(result.column_names)} '
    ...
    def add_column_index(cls, result: PrepActionResult) -> str:
        ...
        row_text = cls._pluralize(result.affected_count, "row")
        return (f'✓ Index column added. {cls._format_column_names(result.column_names)} '
    ...
    def add_column_uuid(cls, result: PrepActionResult) -> str:
        ...
        return (f'✓ UUID column added. {cls._format_column_names(result.column_names)} '
    ...
    def add_column_random(cls, result: PrepActionResult) -> str:
        ...
        return (f'✓ Random column added. {cls._format_column_names(result.column_names)} '
    ...
    def string_function_basic(cls, result: PrepActionResult) -> str:
        ...
        action_text = action_names.get(result.method, f'{result.method.capitalize()} applied')
        value_text = cls._pluralize(result.affected_count, "value")
        return (f'✓ {action_text} from {cls._format_column_names(result.column_names)}. '
    ...
    def string_function_conversion(cls, result: PrepActionResult) -> str:
        ...
        value_text = cls._pluralize(result.affected_count, "value")
        failed_text = f", {result.failed_count} failed conversions" if result.failed_count > 0 else ""
        return (f'✓ {action_text}. {cls._format_column_names(result.column_names)} updated. '
    ...
    def string_function_extract(cls, result: PrepActionResult) -> str:
        ...
        value_text = cls._pluralize(result.remaining_count, "value") if result.remaining_count else ""
        return (f'✓ Pattern extracted. Found {result.value} in {cls._format_column_names(result.column_names)}. '
    ...
    def string_function_dummies(cls, result: PrepActionResult) -> str:
        ...
        return (f'✓ Dummy columns created. {cls._format_column_names(result.column_names)} '
    ...
    def numeric_function(cls, result: PrepActionResult) -> str:
        ...
        action_text = action_names.get(result.method, f'{result.method.capitalize()} applied')
        calculation_text = cls._pluralize(result.affected_count, "calculation")
        return (f'✓ {action_text} to {cls._format_column_names(result.column_names)}. '
    ...
    def datetime_function(cls, result: PrepActionResult) -> str:
        ...
        action_text = extraction_names.get(result.method, f'{result.method.capitalize()} extracted')
        value_text = cls._pluralize(result.affected_count, "value")
        return (f'✓ {action_text}. {cls._format_column_names(result.column_names)} updated. '
    ...
    def delete_by_index(cls, result: PrepActionResult) -> str:
        ...
        return (f'✓ Rows deleted by index. Removed rows {result.additional_info}. '
    ...
    def delete_by_condition(cls, result: PrepActionResult) -> str:
        ...
        remaining_text = cls._pluralize(result.remaining_count, "row")
        return (f'✓ Rows deleted by condition. Removed {result.affected_count} {deleted_text} '
    ...
    def generate_message(cls, result: PrepActionResult) -> str:
        ...
        # Numeric functions
        elif action in ["add", "multiply", "subtract", "divide", "round", "floor", "ceil", "abs"]:
        ...
        # Generic fallback
            ...
            return (f"✓ {action.capitalize()} completed. "
