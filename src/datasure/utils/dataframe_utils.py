...
def standardize_missing_values(data: pd.DataFrame | pl.DataFrame) -> pl.DataFrame:
        ...
        missing_values = [
            ...
            'NULL', 'null', 'Null', 'None', 'none', 'NONE',  # Explicit nulls
            'N/A', 'n/a', 'NA', 'na', '#N/A', 'N/a',  # Not available
            '-', '--', '.', '?', '???',  # Common placeholders
            'Missing', 'missing', 'MISSING',  # Explicit missing
            'Unknown', 'unknown', 'UNKNOWN',  # Unknown values
            'NaN', 'NAN',  # String representations of NaN
            'nan', 'NaT',  # Additional representations
        ]
            ...
            try:
                ...
                if data[col].dtype == pl.Utf8:
                    data = data.with_columns([
                        ...
                        .replace(missing_values, None)  # Replace missing value representations
                        .map_elements(lambda x: None if x == '' else x, return_dtype=pl.Utf8)  # Handle empty strings after stripping
                    ])
                else:
                    # For non-string columns, just replace the missing values
                    data = data.with_columns([
                        pl.col(col).replace(missing_values, None)
                    ])
            except Exception as e:
                # Log warning but continue processing other columns
                print(f"Warning: Could not standardize missing values for column '{col}': {e}")
                continue
