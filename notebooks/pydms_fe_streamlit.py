...
st.logo("IPA-primary-color-CMYK.jpg", size="large", link=None, icon_image=None)

tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8, tab9 = st.tabs(["Data","Summary","Survey Progress", "Duplicates","Enumerator Statistics","Missing Data","Outliers","Back Checks","Descriptive Statistics"])
    ...
    def load_dataframe(file_path):
        if file_path.startswith('http'):
            ...
            if file_path.endswith('.csv'):
                ...
                raise ValueError("Unsupported file format. Please provide a CSV, XLS, XLSX, or DTA file.")
        else:
            # Handle local files
            if file_path.endswith('.csv'):
...
##### Basic set up #####
        ...
        if pd.api.types.is_numeric_dtype(df['aud_consent']):
            ...
            df['recording_consent'] = df['aud_consent'].map({'Yes': 1, 'No': 0}) # If categorical
        ...
        # Define flagged percentage of missings. For example, write 50 if there are more than 50% of missings and should be flagged as warning
...
##### Changes to data #####
    ...
    ##### Back Check Data #####
        ...
        file_path = st.text_input("Insert valid URL or Path for backcheck data (from GitHub or local) if any: ")
        ...
        try:
            if file_path:
                ...
                merge_variable = st.selectbox("Select unique identifier in both datasets:", [col for col in backcheck_df.columns if col in existing_df.columns], index=0)
                ...
                if merge_variable:
                    ...
                    if st.button("Calculate Backchecks"):
                        ...
                        existing_df_copy['original_enum_id'] = existing_df_copy[enumid_var]
                        backcheck_df_copy['original_bcer_id'] = backcheck_df_copy[enum_bcer]
                        ...
                        existing_df_copy.rename(columns=existing_df_columns, inplace=True)
                        backcheck_df_copy.rename(columns=backcheck_df_columns, inplace=True)

                        # Merge datasets
                        merged_df = pd.merge(existing_df_copy, backcheck_df_copy, on=[merge_variable], how='inner')
                        ...
                        merged_df = merged_df.drop(['original_enum_id', 'original_bcer_id'], axis=1)
...
##### Summary #####
        ...
        #### Create first row of value boxes ####
            # Define color codes
            color_completed = "#FFA500" if percentage_finished <= percentage_warning else "#4CAF50"
        ...
        ### Value box 6 ###
            ...
            interviews_per_interviewer = df.groupby('enumid', observed=False).size().reset_index(name='interviews_count')
        ...
        ### Value box 7 ###
            average_interviews_per_day_per_enumerator = average_interviews_per_day / (unique_enumerators) if unique_enumerators > 0 else 0
            rounded_average_day_per_enumerator = round(average_interviews_per_day_per_enumerator, 2)
        ...
        #### Create second row of value boxes
            ...
            today = datetime.now().strftime('%B %d, %Y')  # Format: Month day, year (e.g., August 22, 2024)
...
##### Survey Progress #####
        with tab3:
            ...
            with col2:
                ...
                count_unique_ids = unique_counts.groupby('unique_key_count').count().reset_index()

                # Show result
                st.write("Unique IDs by number of attempts:")

                # Define the color scale
                colors = ['#2C5F2D', '#74AA76', '#9ECED7', '#4D5E90', '#DE9461', '#B9ABE6', '#E0C97D', '#636892']

                # Create the Plotly figure
                fig = go.Figure(data=[go.Pie(labels=count_unique_ids.unique_key_count, values=count_unique_ids["id"], hole=.3, marker=dict(colors=colors))])
            ...
            # Group by date and count number of IDs
            count_by_date = df.groupby('date_only').size().reset_index(name='num_interviews')
            ...
            with col3:
                ...
                table_data['number_of_attempts'] = table_data['id'].map(unique_counts.set_index('id')['unique_key_count'])  

                # Reorder the columns
                table_data = table_data[['number_of_attempts', 'id', 'submissiondate', 'consent']]
                ...
                if query:
                    try:
                        query_num = int(query)
                        filtered_table = table_data[table_data['number_of_attempts'] >= query_num]
                    except ValueError:
            ...
            fig.add_trace(
                go.Scatter(
                    x=[count_by_date['date_only'].min(), count_by_date['date_only'].max()],
...
##### Duplicates #####
        with tab4:
            ...
            selected_col = st.selectbox(
                "Select a variable by which you'd like identify duplicates (id, phone number, etc.)",df4.columns
            )

            # Count duplicates by ID
            df4['num_dups'] = df4[selected_col].map(df4[selected_col].value_counts())

            # Filter rows with duplicates and without missings
            df_duplicates = df4[
                (df4['num_dups'] > 1) & 
            ...
            if df_duplicates.empty:
                ...
                if selected_col == 'id' or selected_col == id_variable:
                    ...
                    result = df_duplicates[['id', selected_col, 'num_dups', 'submissiondate', 'key']]
                ...
                st.dataframe(
                    ...
                    column_config={
                        ...
                        selected_col: st.column_config.Column(
                            width="medium"
                        ),
        ...
        with tab5:
            ...
            with col1:               
                ...
                if filtered_df.shape[0] > 0:
                    average_duration = filtered_df.groupby('enumid', observed=False).agg(
                        avg_duration=('duration_mins', 'mean')
                    ).reset_index()

                    # Sort values by avg_duration
                    average_duration = average_duration.sort_values(by='avg_duration', ascending=True)
                    ...
                    average_duration['std_dev_away'] = (average_duration['avg_duration'] - overall_avg_duration) / std_dev
                    ...
                    fig = px.bar(
                        ...
                        labels={'avg_duration': 'Average Duration (minutes)', 'enumid': 'Enumerator ID'},
                    ...
                    fig.update_traces(hovertemplate="<b>Enumerator ID:</b> %{x}<br>" +
            ...
            with col2:
                ...
                if filtered_df.shape[0] > 0:
                    ...
                    summary = filtered_df.groupby('enumid', observed=False).agg(
                        total_persons=('id', 'size'),
                        consented_persons=('consent', lambda x: (x == 1).sum()),
                    ).reset_index()

                    # Convert to numeric and calculate consent percentage
                    summary['consented_persons'] = pd.to_numeric(summary['consented_persons'], errors='coerce')
                    summary['total_persons'] = pd.to_numeric(summary['total_persons'], errors='coerce')
                    ...
                    if 'merged_df' in locals():
                        ...
                        survey_counts = df1.groupby('enumid').size().reset_index(name='Total Surveys')
                        
                        # Count how many times each enumerator's work was backchecked
                        backcheck_counts = merged_df.groupby('enumid').size().reset_index(name='Backchecked Interviews')
                        ...
                        for svy_col in svy_cols:
                            base_var = svy_col[4:]
                            bck_col = f'bck_{base_var}'
                            if bck_col in merged_df.columns:
                                comparable_vars.append(base_var)
                                prefix_pairs.append((svy_col, bck_col))
                                            
                        # Count mismatches for each enumerator
                        def count_mismatches(group):
                            mismatch_count = 0
                            for svy_col, bck_col in prefix_pairs:
                                svy_values = group[svy_col].astype(str)
                                bck_values = group[bck_col].astype(str)
                                svy_values = svy_values.replace('nan', '')
                                bck_values = bck_values.replace('nan', '')
                                mismatch_count += (svy_values != bck_values).sum()
                            return mismatch_count

                        if len(comparable_vars) > 0:
                            try:
                                # Calculate mismatches by enumerator
                                mismatch_counts = merged_df.groupby('enumid').apply(count_mismatches).reset_index(
                                ...
                                backcheck_data['Backchecked Interviews'] = backcheck_data['Backchecked Interviews'].fillna(0).astype(int)
                                
                                # Calculate total surveys that can be compared
                                backcheck_data['Values Compared'] = (backcheck_data['Backchecked Interviews'] * len(comparable_vars)).astype(int)
                                ...
                                backcheck_data['Total Mismatches'] = backcheck_data['Total Mismatches'].fillna(0).astype(int)
                                
                                # Calculate percentages
                                backcheck_data['Backchecked Percentage'] = (
                                    backcheck_data['Backchecked Interviews'] / backcheck_data['Total Surveys'] * 100
                                ).round(2)
                                
                                backcheck_data['Mismatch Percentage'] = (
                                    backcheck_data['Total Mismatches'] / backcheck_data['Values Compared'] * 100
                                ).round(2)
                                ...
                                combined_summary = combined_summary.drop(['enumid', 'Total Surveys'], axis=1)
                                
                                # Format percentage columns
                                combined_summary['Consent Percentage'] = combined_summary['Consent Percentage'].round(2)
                                combined_summary['Backchecked Percentage'] = combined_summary['Backchecked Percentage'].round(2)
                                combined_summary['Mismatch Percentage'] = combined_summary['Mismatch Percentage'].round(2)
                                ...
                                st.dataframe(
                                    ...
                                    column_config={
                                        ...
                                        "Total Interviews": st.column_config.NumberColumn(format="%d", width="small"),
                                        "Consented Interviews": st.column_config.NumberColumn(format="%d", width="small"),
                                        "Consent Percentage": st.column_config.NumberColumn(format="%.2f%%", width="small"),
                                        "Backchecked Interviews": st.column_config.NumberColumn(format="%d", width="small"),
                                        "Backchecked Percentage": st.column_config.NumberColumn(format="%.2f%%", width="small"),
                                        "Values Compared": st.column_config.NumberColumn(format="%d", width="small"),
                                        "Total Mismatches": st.column_config.NumberColumn(format="%d", width="small"),
                                        "Mismatch Percentage": st.column_config.NumberColumn(format="%.2f%%", width="small")
                                    }
                                )

                                # Display number of comparable variables
                                st.write(f"Note: For backchecks calculations there were found {len(comparable_vars)} comparable variables.")
            ...
            with col3:
                ...
                # Calculate average duration time per enumerator
                average_duration = df5.groupby('enumid', observed=False).agg(
                    avg_duration=('duration_mins', 'mean')
                ).reset_index()
                ...
                df5 = df5.groupby(['date','enumid']).agg({'key': 'nunique', 'duration_mins': 'mean'}).reset_index()
                ...
                dates = pd.DataFrame({'date': df5['date'].unique()}).sort_values('date', ascending=True)
                enums = pd.DataFrame({'enumid': df5['enumid'].unique()}).sort_values('enumid', ascending=True)
                crossjoin = dates.merge(enums, how='cross')

                # Merge the cross join with the original data
                merged = crossjoin.merge(df5, how="left", on=["date", "enumid"]).sort_values(['enumid', 'date'], ascending=[False, True])
                ...
                # Create scatter plot
                fig = px.scatter(
                    merged, x="cumsum_surveys", y="duration_mins", 
                    animation_frame="display_date", color="enumid", animation_group="enumid", 
                    size="cumsum_surveys", hover_name="enumid", 
                ...
                fig.add_shape(
                    type="line",
                    x0=0, y0=current_avg_duration, x1=30, y1=current_avg_duration,
                    line=dict(color="red", width=2, dash="dash"),
        ...
        with tab6:
            def analyze_missing_values(df6, codes, labels):
                ...
                for column in df6.columns:
                    # Count not coded missings (NA, empty spaces, empty strings)
                    not_coded = df6[column].isna().sum() + (df6[column] == " ").sum() + (df6[column] == "").sum()
                    
                    # Initialize dictionary for the row
                    row_dict = {
                        'Variable': column,
                        'Total Missings': not_coded,  # Initialize with not_coded
                        'Not Coded': not_coded
                    }
                    ...
                    for code, label in zip(codes, labels):
                        ...
                        row_dict['Total Missings'] += count  # Add to total
                    
                    # Calculate and add percentages
                    total_rows = len(df6)
                    row_dict['Total Missings (%)'] = (row_dict['Total Missings'] / total_rows) * 100
                ...
                # Create the slider
                missing_threshold = st.slider("Variables with % of missing values above:", 0, 100, 1)
                
                # Filter based on total missing percentage
                missing_data_filtered = missing_data[missing_data['Total Missings (%)'] >= missing_threshold]
                ...
                if not missing_data_filtered.empty:
                    ...
                    with tab1:
                        ...
                        pct_columns = ['Variable', 'Total Missings (%)', 'Not Coded (%)']
                        pct_columns.extend([f'{label} ({code}) (%)' for code, label in zip(codes, labels)])
                    ...
                    with tab2:
                        ...
                        count_columns = ['Variable', 'Total Missings', 'Not Coded']
                        count_columns.extend([f'{label} ({code})' for code, label in zip(codes, labels)])
                    ...
                    st.write("There are no variables with more than", missing_threshold, "% missing values.")

            analyze_missing_values(df6, codes, labels)
            st.write("Note: The calculations for 'Not Coded' includes values of ' . ', 'NA' and empty cells.")
...
##### Outliers #####
        with tab7:
            ...
            with col1:
                ...
                selected_col = st.selectbox('Select a variable to check for outliers',numeric_columns) 
                ...
                st.dataframe(
                    ...
                    column_config={
                        ...
                        "enumid": st.column_config.Column(
                            "Enumid",
                            width="small"
                        ),
            ...
            # Calculate percentage of outliers
                ...
                fig = go.Figure(data=go.Violin(y=df[selected_col], box_visible=True, line_color='black',
                                            meanline_visible=True, fillcolor='darkgreen', opacity=0.6,
                                            x0=selected_col))
            ...
            with col1:
                ...
                def show_pattern_selection(df, numeric_columns):
                    ...
                    pattern_options = [f"{pattern} ({len(cols)} variables)" for pattern, cols in pattern_groups.items()]
                    pattern_to_base = {display: pattern for pattern, display in zip(pattern_groups, pattern_options)}
                    ...
                    if selected_pattern:
                        ...
                        df_melted = pd.melt(df_subset, id_vars=['id'], value_vars=selected_cols, var_name='name_variable', value_name='new_var')
                ...
                if selected_cols and df_melted is not None:
                    ...
                    st.dataframe(
                        ...
                        column_config={
                            ...
                            "new_var": st.column_config.NumberColumn("Value", format="%.2f", width="small"),
                            "mean": st.column_config.NumberColumn("Mean",format="%.2f", width="small"),
                            "lower_bound": st.column_config.NumberColumn("Lower Bound",format="%.2f", width="small"),
                            "upper_bound": st.column_config.NumberColumn("Upper Bound",format="%.2f", width="small")
                        }
                    )
            ...
            with col2:
                ...
                fig = go.Figure(data=go.Violin(y=df_melted["new_var"], box_visible=True, line_color='black',
                                                meanline_visible=True, fillcolor='forestgreen', opacity=0.6,
                                                x0=x_axis_label))
...
##### Back Checks #####
        with tab8:
            if 'merged_df' in st.session_state:
                ...
                bck_vars = [col for col in merged_df.columns if col.startswith('bck_')]

                # Create pairs of variables that exist in both datasets
                var_pairs = []
                for svy in svy_vars:
                    base_name = svy[4:]  
                    bck_name = f'bck_{base_name}'
                    if bck_name in bck_vars:
                        var_pairs.append(base_name)

                # Allow the user to select variables to compare
                selected_vars = st.multiselect("Select variables to compare:", var_pairs)

                # Create tabs for each selected variable
                if selected_vars:
                    tabs = st.tabs(["General Results","Enumerator Statistics","Backchecker Statistics"] + selected_vars)
                    var_summary = {}
                    mismatches_dict = {}
                    ...
                    with tabs[0]:
                        ...
                        enum_bcer_column = enum_bcer[0] if isinstance(enum_bcer, list) else enum_bcer
                        ...
                        for var in selected_vars:
                            ...
                            comparison_df = pd.DataFrame({
                                ...
                                'Backchecker id': merged_df[enum_bcer_column] if enum_bcer_column in merged_df.columns else pd.Series([None] * len(merged_df)),
                                'Survey Value': merged_df[svy_col].astype(str),  
                                'Backcheck Value': merged_df[bck_col].astype(str)  
                            })

                            comparison_df['Comparison'] = comparison_df.apply(
                                lambda x: 'Match' if str(x['Survey Value']).strip() == str(x['Backcheck Value']).strip() else 'Mismatch',
                                axis=1
                            )

                            mismatches_df = comparison_df[comparison_df['Comparison'] == 'Mismatch']
                            ...
                            var_summary[var] = {
                                ...
                                'mismatch_percentage': (len(mismatches_df) / len(comparison_df) * 100) if len(comparison_df) > 0 else 0
                            }
                        ...
                        enumerator_stats.columns = ['Selected Variables', 'Variable Type', 'Total Surveys', 'Total Backchecks', '# Compared', '# Different', '% Different']

                        # Format the "% different" column
                        enumerator_stats['% Different'] = (enumerator_stats['% Different'].round(2)).astype(str) + '%'
                    ...
                    with tabs[1]:
                        ...
                        for var in selected_vars:
                            ...
                            comparison = pd.DataFrame({
                                ...
                                'backcheck_value': merged_df[bck_col].astype(str)
                            })
                            
                            # Mark matches/mismatches
                            comparison['is_different'] = comparison.apply(
                                lambda x: str(x['survey_value']).strip() != str(x['backcheck_value']).strip(),
                                axis=1
                            )
                            ...
                            for enum_id in comparison['enumerator_id'].unique():
                                ...
                                if var == selected_vars[0]:  # First variable only
                                    enumerator_detailed_stats[enum_id]['total_backchecks'] = len(enum_data)
                                
                                enumerator_detailed_stats[enum_id]['total_compared'] += len(enum_data)
                                enumerator_detailed_stats[enum_id]['total_different'] += enum_data['is_different'].sum()
                        
                        # Create DataFrame from enumerator statistics
                        enumerator_detailed_df = pd.DataFrame.from_dict(enumerator_detailed_stats, orient='index')
                        enumerator_detailed_df = enumerator_detailed_df.reset_index()
                        enumerator_detailed_df.columns = ['Enumerator ID', 'Total Surveys', 'Total Backchecks', 'Total Values Compared', 'Total Different']
                        
                        # Calculate percentages
                        enumerator_detailed_df['% Backchecked'] = (
                            (enumerator_detailed_df['Total Backchecks'] / enumerator_detailed_df['Total Surveys'] * 100)
                            .round(2)
                        )
                        
                        enumerator_detailed_df['% Different'] = (
                            (enumerator_detailed_df['Total Different'] / enumerator_detailed_df['Total Values Compared'] * 100)
                            .round(2)
                        )
                        
                        # Handle division by zero
                        enumerator_detailed_df['% Backchecked'] = enumerator_detailed_df['% Backchecked'].fillna(0)
                        enumerator_detailed_df['% Different'] = enumerator_detailed_df['% Different'].fillna(0)
                        
                        # Convert percentages to strings with % symbol
                        enumerator_detailed_df['% Backchecked'] = enumerator_detailed_df['% Backchecked'].astype(str) + '%'
                        enumerator_detailed_df['% Different'] = enumerator_detailed_df['% Different'].astype(str) + '%'
                        ...
                        selected_enumerators = st.multiselect("Filter enumerators:", enumerator_detailed_df['Enumerator ID'].unique())

                        if selected_enumerators:
                            filtered_enumerator_stats = enumerator_detailed_df[enumerator_detailed_df['Enumerator ID'].isin(selected_enumerators)]
                        else:
                        ...
                        st.dataframe(
                            ...
                            column_config={
                                "Enumerator ID": st.column_config.Column(width="small"),
                                "Total Surveys": st.column_config.NumberColumn(format="%d", width="small"),
                                "Total Backchecks": st.column_config.NumberColumn(format="%d", width="small"),
                                "% Backchecked": st.column_config.Column(width="small"),
                                "Total Values Compared": st.column_config.NumberColumn(format="%d", width="small"),
                                "Total Different": st.column_config.NumberColumn(format="%d", width="small"),
                    ...
                    with tabs[2]:                        
                            ...
                            for backchecker_id in comparison['backchecker_id'].unique():
                                if backchecker_id not in backchecker_stats:
                                    backchecker_stats[backchecker_id] = {
                                        'total_backchecks': len(comparison[comparison['backchecker_id'] == backchecker_id]),
                                        'total_compared': 0,
                                        'total_different': 0
                                    }
                                
                                backchecker_data = comparison[comparison['backchecker_id'] == backchecker_id]
                                backchecker_stats[backchecker_id]['total_compared'] += len(backchecker_data)
                                backchecker_stats[backchecker_id]['total_different'] += backchecker_data['is_different'].sum()
                        
                        # Create DataFrame from backchecker statistics
                        backchecker_df = pd.DataFrame.from_dict(backchecker_stats, orient='index')
                        backchecker_df = backchecker_df.reset_index()
                        backchecker_df.columns = ['Backchecker ID', 'Total Backchecks', 'Total Values Compared', 'Total Different']
                        
                        # Calculate percentage different
                        backchecker_df['% Different'] = (
                            (backchecker_df['Total Different'] / backchecker_df['Total Values Compared'] * 100)
                            .round(2)
                            .astype(str) + '%'
                        )
                        
                        st.subheader("Backchecker Statistics")
                        selected_backcheckers = st.multiselect("Filter backcheckers:", backchecker_df['Backchecker ID'].unique())

                        if selected_backcheckers:
                            filtered_backchecker_stats = backchecker_df[backchecker_df['Backchecker ID'].isin(selected_backcheckers)]
                        else:
                        ...
                        st.dataframe(
                            ...
                            column_config={
                                ...
                                "Total Backchecks": st.column_config.NumberColumn(format="%d", width="medium"),
                                "Total Values Compared": st.column_config.NumberColumn(format="%d", width="medium"),
                                "Total Different": st.column_config.NumberColumn(format="%d", width="medium"),
                        ...
                        with tab:
                            ...
                            if is_numeric:
                                try:
                                    ...
                                    survey_numeric = pd.to_numeric(merged_df[svy_col], errors='coerce')
                                    backcheck_numeric = pd.to_numeric(merged_df[bck_col], errors='coerce')
                                    
                                    # Add difference column
                                    comparison_df['Difference'] = survey_numeric - backcheck_numeric
                                    ...
                                    st.warning(f"Could not calculate differences for {var}. Error: {str(e)}")
                            ...
                            selected_enumerators = st.multiselect(f"Filter mismatches for {var} by enumerator:", mismatches_df['Enumerator id'].unique())
                            if selected_enumerators:
                                filtered_mismatches = mismatches_df[mismatches_df['Enumerator id'].isin(selected_enumerators)]
                            else:
                ...
                st.info("There was no backcheck data frame found. Please upload the backcheck data first.")
